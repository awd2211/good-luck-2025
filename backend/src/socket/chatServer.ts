...
const registerAgentEvents = (socket: Socket) => {
  ...
  socket.on('agent:online', async (data) => {
    try {
      ...
      // 更新客服状态
      await csAgentService.updateAgentStatus(agentId, 'online');
  ...
  socket.on('agent:offline', async (data) => {
    try {
      ...
      // 更新客服状态
      await csAgentService.updateAgentStatus(agentId, 'offline');
  ...
  socket.on('agent:busy', async (data) => {
    try {
      ...
      // 更新客服状态
      await csAgentService.updateAgentStatus(agentId, 'busy');
...
const startTimeoutCleaner = (): void => {
  ...
  console.log('[Timeout] Timeout cleaner started, checking every 5 minutes');
};
